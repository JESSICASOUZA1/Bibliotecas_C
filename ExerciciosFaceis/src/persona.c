...
void hardCode(Persona* pArray,int limite)
{
 ...
 */
int initializeArrayPersonas(Persona* pArray,int limite) //Inicializar
{
	int retorno = -1;
	int i;
 ...
 */
int findEmpty(Persona* pArray,int limite) //BuscarLibre
{
	int retorno = -1;
    int i;
...
int altaArrayPersona(Persona* pArray,int limite)
{
	int retorno = -1;
	...
	if(pArray != NULL && limite>0)
	{
		if(utn_getNombre(BufferPersona.nombre,ARRAY_LEN_NOMBRE,"\nIngrese nombre: ","\nError,nombre invalido.\n",QTY_REINTENTOS) &&
				utn_getApellido(BufferPersona.apellido,ARRAY_LEN_APELLIDO,"\nIngrese apellido: ","\nError,apellido invalido.\n",QTY_REINTENTOS) &&
				utn_getDni(BufferPersona.dni,ARRAY_LEN_DNI,"\nIngrese DNI: ","\nError,DNI invalido",QTY_REINTENTOS) == 0)
		{
